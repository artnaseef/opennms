...
public class InterfaceToNodeCacheDaoImplIT implements InitializingBean {
    ...
    public void testSetIdWithDifferentNodes() throws Exception {
        ...
        Assert.assertTrue(m_cache.getNodeId(MonitoringLocationDao.DEFAULT_MONITORING_LOCATION_ID, iface.getIpAddress()).contains(node.getId()));
        Assert.assertTrue(m_cache.getNodeId(MonitoringLocationDao.DEFAULT_MONITORING_LOCATION_ID, iface.getIpAddress()).contains(m_databasePopulator.getNode1().getId()));
